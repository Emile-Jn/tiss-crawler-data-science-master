...
class crawler:
    ...
    # Added by Emile Johnston
    def extract_fundamentals(self, driver, URL):
        """
        Extract URLs from grid cells in the specified range between two course names.

        Parameters:
        - driver: the Selenium WebDriver instance
        - URL: the URL of the webpage to fetch data from

        Returns:
        - A list of URLs found within the specified range in the table.
        """
        driver.get(URL)
        time.sleep(self.sleeptime_fetchpage)  # Ensure page loads fully

        # Identify start and end cells by course name
        start_course = "FDS/FD - Fundamentals of Data Science - Foundations"
        end_course = "MLS/FD - Machine Learning and Statistics - Foundations"

        urls_in_range = []
        in_target_range = False

        # Locate the table containing the courses
        try:
            table = driver.find_element(By.XPATH, "//table[@role='grid']/tbody")
            rows = table.find_elements(By.TAG_NAME, "tr")

            for row in rows:
                cell_text = row.text

                # Set in_target_range to True once start_course is found
                if start_course in cell_text:
                    in_target_range = True

                # If within target range, check for URL
                if in_target_range:
                    try:
                        link = row.find_element(By.TAG_NAME, "a")
                        if link.get_attribute("href").startswith("http"):
                            urls_in_range.append(link.get_attribute("href"))
                    except NoSuchElementException:
                        pass

                # End extraction once end_course is found
                if end_course in cell_text:
                    in_target_range = False
                    break
        except NoSuchElementException as e:
            print("Table or row elements not found:", e)
